...
//week3
int enemyPosX1,enemyPosY1;
int enemyPosX2,enemyPosY2;
int enemyPosX3,enemyPosY3;
...
void draw() {
  ...
  switch (gameState){
      ...
      case GAME_RUN:
        ...
        for(int i=0;i<5;i++){
        image(enemyImage,enemyPosX1%(width*3)-(spacing*i),enemyPosY1);//三輪飛機所以x3  
        }
        for(int i=0;i<5;i++){
        image(enemyImage,enemyPosX2%(width*3)-(spacing*i),enemyPosY2+(spacing*i));//三輪飛機所以x3  
        }
        
        //速成菱形
        image(enemyImage,enemyPosX3%(width*3),enemyPosY3);
